const options = [
  {
    title: "Ofertas",
    href: "./index.html",
    refs: ["Laptops", "Audio", "Auticulares"],
  },
  {
    title: "Cómo comprar",
    href: "./index.html",
    refs: ["Formas de pago", "Envios", "Devoluciones"],
  },
  {
    title: "Costos y tarifas",
    href: "./index.html",
    refs: ["Impuestos", "Facturación"],
  },
  {
    title: "Mis pedidos",
    href: "./index.html",
    refs: ["Pedir nuevamente", "Lista de deseos"],
  },
  { title: "Garantía", href: "./index.html", refs: [] },
];

function hideSearch() {
  const pathname = location.pathname;
  if (!pathname.includes("index")) {
    document
      .querySelector(".header-form")
      .removeChild(document.querySelector(".header-input"));
  }
}

function printNavBar(opts, id) {
  let template = "";
  for (const each of opts) {
    template =
      template +
      `
      <li class="nav-li">
        <a class="nav-a" href="${each.href}">${each.title}</a>
      </li>
    `;
  }
  const selector = document.getElementById(id);
  selector.innerHTML = template;
}

function printFooter(opts, id) {
  let template = "";
  for (const each of opts) {
    template =
      template +
      `
        <ul class="footer-ul">
        <li class="footer-main-item">
          <a class="footer-a" href="./index.html">${each.title}</a>
        </li>
        ${each.refs
          .map(
            (ref) =>
              `<li class="footer-li"><a class="footer-a" href="./index.html">${ref}</a></li>`
          )
          .join("")}
      </ul>
    `;
  }
  const selector = document.getElementById(id);
  selector.innerHTML = template;
}

function printIcons() {
  let template = `
      <li id="facebook" class="header-li">
        <a class="header-a" href="https://facebook.com">
          <img
            class="header-social-img"
            src="data:image/png;base64,"
          />
        </a>
      </li>
      <li id="instagram" class="header-li">
        <a class="header-a" href="https://instagram.com">
          <img
            class="header-social-img"
            src="data:image/png;base64,"
          />
        </a>
      </li>
      <li id="cart" class="header-li">
        <a class="header-a" href="./cart.html">
          <img
            class="header-social-img"
            src="data:image/png;base64,"
          />
        </a>
      </li>
      <li id="login" class="header-li">
        <a id="iconSesion" class="header-a"" href="#">
          <img id="imgClose"
            class="header-social-img"
            src="data:image/png;base64,"
          />
        </a>
      </li>
    `;
  document.querySelector(".header-social").innerHTML = template;
}

// Paso 2: Definir la función para renderizar los íconos
function renderizarIconos() {
  const carritoIcono = document.getElementById("cart");
  const imgClose = document.getElementById("imgClose");
  const imgOpen = document.getElementById("imgOpen");

  const isOnline = JSON.parse(localStorage.getItem("isOnline"));
  console.log(isOnline);
  if (isOnline) {
    carritoIcono.style.display = "inline-block";
    imgClose.remove();
    document.querySelector("#iconSesion").innerHTML  = `    
        <img id="imgOpen"
          class="header-social-img"
          src="data:image/png;base64,"
        /> 
      `;
  } else {
    carritoIcono.style.display = "none";
    imgOpen.remove();
    document.querySelector("#iconSesion").innerHTML = `
        <img id="imgClose"
          class="header-social-img"
          src="data:image/png;base64,"
        />
      `;
  }
}

/*const isOnline = false;
function printIconLogin() {
  const removerIconoLogin = document.getElementById("iconSesion");
  removerIconoLogin.remove();
  if (isOnline) {
    let templateIconLogin = `
    <a id="iconSesion" class="header-a" href="#">
      <img
        class="header-social-img"
        src="data:image/png;base64,"
      />
    </a>`;
    document.querySelector(".login").innerHTML = templateIconLogin;
  } else {
    let templateIconLogin = `
    <a id="iconSesion" class="header-a" href="#">
      <img
        class="header-social-img"
        src="data:image/png;base64,"
      />
    </a>`;
    document.querySelector(".login").innerHTML = templateIconLogin;
  }
}

function isOnline() {
  // Seleccionar el enlace por su ID
  const enlace = document.getElementById("iconSesion");

  // Agregar un event listener para el evento de clic
  enlace.addEventListener("click", function (event) {
    // Prevenir el comportamiento predeterminado del enlace (navegar a una nueva página)
    event.preventDefault();

    // Ejecutar la acción deseada aquí
    if (isOnline) {
      isOnline = false;
    } else {
      isOnline = true;
    }
  });

  console.log(isOnline);
}*/
